...
def click_movie_btn_select():
    ...
    lTitle, lCountry, lGenre, lEnglish, lHour_viewed = [], [], [], [], []
    ...
    # 2) select 해온 데이터 insert
    for item1, item2, item3, item4, item5 in zip(lTitle, lCountry, lGenre, lEnglish, lHour_viewed):
        listTitle.insert(END, item1)
        listCountry.insert(END, item2)
        listGenre.insert(END, item3)
        listEnglish.insert(END, item4)
        listHour_viewed.insert(END, item5)
...
btn_TVpro = Button(window, text="영화", fg="red", bg="yellow", command=click_movie_btn_select)
btn_TVpro.pack(side = TOP, padx = 10, pady = 10)
